...
const session: TranscriptionSession = {
  ...
  stoppedAt: '2026-07-08T10:11:00.000Z',
...
describe('exportService', () => {
  it('produces a UTF-8 TXT transcript with metadata, speakers and roles', () => {
    ...
    expect(text).toContain('القاضي — أحمد الحربي');
    expect(text).toContain('المدعي — خالد');
    expect(text).toContain('افتتحت الجلسة.');
  });

  it('names the file from the case number when present', () => {
    const result = exportService.export(session, segments, 'txt');
    expect(result.filename).toContain('٤٣٥');
  });
  ...
  it('excludes interim (non-final) segments from the export', () => {
    const withInterim: TranscriptSegment[] = [
      ...segments,
      { ...segments[0]!, id: '44444444-4444-4444-4444-444444444444', text: 'مؤقت', isFinal: false },
    ];
    const text = exportService.export(session, withInterim, 'txt').buffer.toString('utf-8');
    expect(text).not.toContain('مؤقت');
  });
});
